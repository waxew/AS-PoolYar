            ...
            TopAppBar(
                ...
                actions = {
                    Button(
                        ...
                        enabled = uiState.lines.isEmpty() || uiState.config.columnSeparator.isNotEmpty(),
                    ) {
                        Text(
                            text = if (uiState.lines.isEmpty()) {
                                stringResource(localesR.string.select_file)
                            } else {
                                stringResource(localesR.string.import_title)
                            },
                        )
                    }
                },
            ...
            Column(
                ...
                if (uiState.lines.isNotEmpty()) {
                    ...
                    MappingField(
                        ...
                        onColumnSelected = { onConfigUpdate(uiState.config.copy(dateColumnIndex = it)) },
                    )
                    MappingField(
                        label = stringResource(localesR.string.amount),
                        selectedIndex = uiState.config.amountColumnIndex,
                        columns = uiState.columns,
                        onColumnSelected = { onConfigUpdate(uiState.config.copy(amountColumnIndex = it)) },
                    )
                    MappingField(
                        label = stringResource(localesR.string.description),
                        selectedIndex = uiState.config.descriptionColumnIndex,
                        columns = uiState.columns,
                        onColumnSelected = { onConfigUpdate(uiState.config.copy(descriptionColumnIndex = it)) },
                    )
                    MappingField(
                        label = stringResource(localesR.string.category_title),
                        selectedIndex = uiState.config.categoryColumnIndex,
                        columns = uiState.columns,
                        onColumnSelected = { onConfigUpdate(uiState.config.copy(categoryColumnIndex = it)) },
                    )
